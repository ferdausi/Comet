...
if ( ! class_exists( 'comet_Menu_Walker' ) ):

	class comet_Menu_Walker extends Walker_Nav_Menu {
		...
		public static function fallback( $args ) {
			if ( current_user_can( 'manage_options' ) ) {
				$fb_output = '<li class="menu-item"><a href="' . esc_url( home_url( '/' ) ) . '">' . esc_html__( 'Home', 'breakingcomet' ) . '</a></li>';
				$fb_output .= '<li class="menu-item"><a target="_blank" href="' . esc_url( admin_url( 'nav-menus.php' ) ) . '">' . esc_html__( 'Add Menu', 'breakingcomet' ) . '</a></li>';
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( function_exists( 'pstudio_get_menu_meta' ) ) {
				$load_sidebar      = pstudio_get_menu_meta( $item->ID, 'widgets' );
				$load_column_class = pstudio_get_menu_meta( $item->ID, 'menucolumnclass', 'col-md-10' );

				$menu_heading               = pstudio_get_menu_meta( $item->ID, 'menuheading', '' );
				$menu_icon_color            = pstudio_get_menu_meta( $item->ID, 'iconcolor', '' );
				$menu_icon_background_color = pstudio_get_menu_meta( $item->ID, 'iconbackgroundcolor', '' );
				$menu_icon                  = pstudio_get_menu_meta( $item->ID, 'icon', '' );
				$menu_background_image_id   = pstudio_get_menu_meta( $item->ID, 'menubackgroundimage', '' );
			}
			...
			if ( isset( $args->has_children ) and $args->has_children and 0 === $depth ) {
				$item_output .= ' <span class="custom-caret"></span>';
			}
			...
			if ( $load_sidebar and is_active_sidebar( $load_sidebar ) ) {
				...
				$item_output .= '
                            ...
                            <ul class="label' . ( $depth + 1 ) . ' sub-dropdown-menu">
                                <li>
                                    <div class="megamenu-container ' . apply_filters( 'pstudio_megamenu_container_class', '' ) . '"><!-- .container class removed -->
                                        <div class="row">';
